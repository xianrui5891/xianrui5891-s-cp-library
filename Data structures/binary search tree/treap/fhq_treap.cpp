...
public:
    ...
    inline void insert(const shared_ptr<node>& ind){//ind->val可能不存在
        auto&& [x,y]=split_by_pred(ind->val,rt,strong_comp);
        rt=merge(merge(x,ind),y),set_tree();
    }
    inline void insert(uint k,const shared_ptr<node>& ind){
        auto&& [x,y]=split_by_rk(k,rt);
        rt=merge(merge(x,ind),y),set_tree();
    }//插到k+1位置上，标号1开始
    ...
    inline void erase(const shared_ptr<node>& ind){//必然存在
        uint rk_=get_rk(ind);
        auto&& [x,t]=split_by_rk(rk_-1,rt);
        auto&& [y,z]=split_by_rk(ind->cnt,t);
        rt=merge(x,z),set_tree();
    }//删除相关的指针，哪怕有多个
    inline void erase(uint k){
        auto&& [x,t]=split_by_rk(k-1,rt);
        auto&& [y,z]=split_by_rk(1,t);
        rt=merge(x,z),set_tree();
    }//只删除排名为k的（1个val）
    inline void erase(const answer_type& val){
        auto&& [x,t]=split_by_pred(val,rt,strong_comp);
